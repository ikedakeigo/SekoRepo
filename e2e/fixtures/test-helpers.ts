...
// ── テスト用アカウント情報 ──
// .env.test または環境変数から読み込む
export const STAFF_USER = {
  email: process.env.E2E_STAFF_EMAIL ?? "staff-test@example.com",
  password: process.env.E2E_STAFF_PASSWORD ?? "testpass1",
  name: "テストスタッフ",
};
...
export function screenshotPath(testName: string, fileName: string): string {
...
export function getTestImagePath(): string {
  ...
  const testImagePath = path.join(testImageDir, "test-photo.jpg");
